...
class UserRegisterRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'username' => ['required', 'string', 'max:255', 'unique:users', 'bail'],
            'email' => ['required', 'string', 'email', 'max:255', 'unique:users', 'bail'],
            'password' => ['required', 'string', 'min:12', 'confirmed', 'bail'],
            'contact_number' => ['required', 'integer', 'max:255', 'unique:users', 'bail'],
            'bio' => ['nullable', 'string', 'bail'],

            'street_name' => ['required', 'string', 'max:255', 'bail'],
            'city_name' => ['required', 'string', 'max:255', 'bail'],
            'state_province' => ['required', 'string', 'max:255', 'bail'],
            'postal_code' => ['required', 'integer', 'max:255', 'bail'],
            'country_name' => ['required', 'string', 'max:255', 'bail'],
        ];
    }
